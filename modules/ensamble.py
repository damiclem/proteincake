...
"""
def ensemble(pssm_path, hmm_path, true_positive_path):
    # Get PSSM results
    ...
    ensemble_positive = index_pssm.union(index_hmm)
    ensemble_df = pd.DataFrame()
    PSSM = [x in index_pssm for x in ensemble_positive]
    HMM = [x in index_hmm for x in ensemble_positive]
    TP = [x in true_positive for x in ensemble_positive]
    ensemble_df['PSSM'] = PSSM
    ensemble_df['HMM'] = HMM
    ensemble_df['TP'] = TP
    ensemble_df.index = ensemble_positive
    return ensemble_df

"""
Create the confusion matrix and compute some statistics (precision, recall, accuracy)
using the results of the ensemble and the positive and negative test set
(e.g results/positive_pfam.fasta results/negative_pfam.fasta).
Return a numpy matrix for the confusion_matrix and dictionary
"""
def confusion_matrix(ensemble_df, positve_path, negative_path):
    # Positive IDs Sets
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_path',         type=str,   default='results/ensemble/ensemble_df.csv')
    parser.add_argument('--out_path_img',     type=str,   default='results/ensemble/confusion_matrix.png')
    parser.add_argument('--out_path_metrics', type=str,   default='results/ensemble/metrics.csv')
    ...
    # 3. Run the ensemble
    df = ensemble(args.pssm_df_path, args.hmm_df_path, args.positive_path)
